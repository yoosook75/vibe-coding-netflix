...
function getCertification(show) {
  const krRating = show.content_ratings?.results?.find((r) => r.iso_3166_1 === 'KR');
  const cert = krRating?.rating;
...
function renderInfoMeta(show) {
  const cert = getCertification(show);
  const genres = (show.genres || []).map((g) => g.name).join(', ');
  const runtime = show.episode_run_time?.[0] ? `${show.episode_run_time[0]}분` : '';
...
function renderShow(show) {
  currentShow = show;
  document.title = `YSCine - ${show.name}`;

  const year = show.first_air_date ? show.first_air_date.slice(0, 4) : '';
  movieEnTitle.textContent = `${show.original_name || show.name}${year ? `, ${year}` : ''}`;
  movieTitle.textContent = show.name;

  if (show.poster_path) {
    const posterUrl = `${IMAGE_BASE}${show.poster_path}`;
    moviePoster.src = posterUrl;
    moviePoster.alt = `${show.name} 포스터`;
    moviePosterThumb.src = posterUrl;
    moviePosterThumb.alt = '';
  }

  const creator = show.created_by?.[0]?.name
    || show.credits?.crew?.find((c) => c.job === 'Executive Producer')?.name
    || '-';
  const cast = (show.credits?.cast || []).slice(0, 4).map((c) => c.name).join(', ') || '-';

  movieDirector.textContent = creator;
  movieCast.textContent = cast;
  renderInfoMeta(show);
  movieRelease.textContent = formatReleaseDate(show.first_air_date);
  movieOverview.textContent = show.overview || '줄거리 정보가 없습니다.';
  ...
  fetchTvImages(show.id);
  fetchTvReviews(show.id);
}
...
function openVideoModal(video) {
  ...
  const title = currentShow ? `<${currentShow.name}> ${video.name}` : video.name;
...
async function fetchTvReviews(tvId) {
  try {
    const response = await fetch(
      `https://api.themoviedb.org/3/tv/${tvId}/reviews?api_key=${API_KEY}&language=ko-KR&page=1`
    );
    if (!response.ok) {
      renderReviews([]);
      return;
...
async function fetchTvVideos(tvId) {
  ...
  try {
    const playable = await loadPlayableVideos(`tv/${tvId}`);
...
async function fetchTvImages(tvId) {
  try {
    const response = await fetch(
      `https://api.themoviedb.org/3/tv/${tvId}/images?api_key=${API_KEY}`
    );
...
async function fetchTvDetail(tvId) {
  const url = `https://api.themoviedb.org/3/tv/${tvId}?api_key=${API_KEY}&language=ko-KR&append_to_response=credits,content_ratings`;
  ...
  try {
    ...
    showError(`TV 시리즈 정보를 불러오지 못했습니다. ${err.message}`);
  }
}
...
async function init() {
  ...
  const tvId = getTvId();
  if (!tvId) {
    showError('TV 시리즈 ID가 없습니다. 목록에서 시리즈를 선택해 주세요.');
  } else {
    fetchTvDetail(tvId);
  }
}
